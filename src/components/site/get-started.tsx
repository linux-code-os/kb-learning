      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeading
          ...
          title={
            <>
              {t("start.title1")}{" "}
              <span className="text-gradient-brand">{t("start.titleAccent")}</span>
            </>
          }
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-5">
            ...
            <Tabs defaultValue={buildInstructions[0].id}>
              ...
              {buildInstructions.map((item) => (
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground">{pick(item.note, lang)}</p>
                </TabsContent>
              ))}
            </Tabs>
          ...
          >
            <div className="h-full rounded-2xl border border-border/60 bg-card/60 p-6">
              <h3 className="text-base font-bold">{t("start.prereqTitle")}</h3>
              <ul className="mt-5 space-y-4">
                {prerequisites.map((p) => (
                  <li
                    key={pick(p.title, lang)}
                  ...
                  >
                    ...
                    <div>
                      <div className="text-sm font-semibold">{pick(p.title, lang)}</div>
